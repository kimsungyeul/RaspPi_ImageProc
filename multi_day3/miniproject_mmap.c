...
int main(int argc, char** argv)
{
	...
	pfb = (unsigned short*)mmap(0, fbvar.xres_virtual* fbvar.yres_virtual
			*NO_OF_COLOR, PROT_READ | PROT_WRITE, MMAP_SHARED, fd, 0);
	...
	for(j=0;j<fbvar.yres_virtual;j++) {
		for(i=(fbvar.xres/3)*2;i<fbvar.xres_virtual;i++){
			offset = (i + j*fbvar.xres)*NO_OF_COLOR;
			lseek(fd, offset, SEEK_SET);
	...
	munmap(pfb, fbvar.xres*fbvar.yres); 	
	close(fd);
	return 0;
}
